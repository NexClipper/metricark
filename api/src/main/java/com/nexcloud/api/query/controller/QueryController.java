...
public class QueryController extends SpringBootServletInitializer implements Serializable, WebApplicationInitializer {
	...
	* @throws Exception
	*/
	@ApiOperation(value="Dynamic Query", httpMethod="POST", notes="동적 쿼리 작성")
	@ApiImplicitParams({
		@ApiImplicitParam(
				name = "cluster_id",
	...
	@RequestMapping(value="/cluster/{cluster_id}/query", method=RequestMethod.POST)
	@ResponseBody
	 ...
	 * @throws Exception
	 */
	@ApiOperation(value="Redis GET TEst", httpMethod="POST", notes="")
	@ApiImplicitParams({
			@ApiImplicitParam(
					name = "cluster_id",
					value = "Cluster ID (ex) 1",
	...
	@RequestMapping(value="/cluster/{cluster_id}/redis/key/{key}/field/{field}", method=RequestMethod.POST)
	@ResponseBody
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
